...
from score_ranking_vote import score_ranking_knn, get_instance_distance
...
def prepare_dataset(group_id, topic_list, gaptime, pop_level, prediction_date, target_date, alpha, percentage_threshold):
    ...
    percentage_threshold: viral or not 
    ...
    for topic_id in topic_list:
        ...
        for line in f:
            ...
            feature = [current_comment_count,num_authors, diffusion_depth, mean_degree, avg_weighted_depth_sum]
...
def classify(train_set, test_set, k, num_level, effective_factors):
    """ 
    ...
    for test_topic_id, test_ins, true_level in test_set:
        ...
        nearest_neighbor_level, knn_topic_id, weighted_num_comment = effective_factor_knn(train_set, test_ins, k, effective_factors)
...
def main(group_id):
    ...
    effective_factors = find_effective_factor(train_set, k)
    y_true, y_pred, comment_true, comment_pred = classify(train_set, test_set, k, num_level, effective_factors)
    # evaluate results
    classification_evaluation(y_true, y_pred)
    level_MSE_evaluation(y_true, y_pred)
    save_correct_predictions(test_set, y_true, y_pred, factor_name = 'current_comment_count')
