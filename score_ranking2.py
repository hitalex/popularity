...
def gen_score_list(topic_score, topic_popularity):
    ...
    # 按照得分作为weight，从高到低排序
    score_list.sort(key=operator.itemgetter(0), reverse=True)
...
def score_ranking_knn(test_ins, train_set, k, gamma = 1):
    ...
    for findex in range(num_feature):
        ...
        for train_topic_id, train_ins, level in train_set:
            ...
            distance_comment_list[index] = [train_topic_id, weight, target_comment_count]
            index += 1
            
        # 按照weight高低进行降序排序
        distance_comment_list.sort(key=operator.itemgetter(1), reverse=True)
        
        for i in range(train_count):
            topic_id = distance_comment_list[i][0]
            weight = distance_comment_list[i][1] / weight_sum
            # 根据排名计算得分，得到自己的weight作为score
            topic_score_feature[topic_id] = weight
