...
def prepare_dataset(group_id, topic_list, gaptime, pop_level, prediction_date, target_date, alpha):
    ...
    for topic_id in topic_list:
        ...
        for line in f:
            ...
            if pubdate >= thread_pubdate + target_date:
                ...
                # 根据当前的comment数量，确定其level
                popularity_level = get_level_index(current_comment_count, pop_level)
            
            # 最多只考虑 MAX_COMMENT 个评论
            if len(comment_feature_list) > MAX_COMMENT:
                break
                
        #Note: 有可能帖子最后一个评论的时间距离发布时间还不足 prediction_date，则可以放弃这种帖子
        if not prediction_date_flag:
            continue
        # 如果在预测时的comment数量小于MIN_COMMENT_PREDICTION_DATE，则放弃预测
        if prediction_comment_count < MIN_COMMENT_PREDICTION_DATE:
            continue
        # 如果最后一个评论的时间还不到target date
        if not target_date_flag:
            target_comment_count = total_comment
            popularity_level = get_level_index(total_comment, pop_level)
            
        level_count_list[popularity_level] += 1
        ...
        # 获得topic的category
        #cat = get_topic_category(thread_pubdate, comment_feature_list, threshold=0.4)
        if target_comment_count >= prediction_comment_count * alpha:
            cat = 1
        else:
            cat = 0
        ...
        comment_count_feature[0][1] = topic_feature[0][1] = int(prediction_date.total_seconds() / gaptime.total_seconds())
        # 设置为最后一个interval
...
def save_filtered_topics(group_id, dataset):
    ...
    """
    path = 'data-dynamic/TopicList-filtered-' + group_id + '.txt'
    f = open(path, 'w')
...
def save_correct_predictions(test_set, y_true, y_pred, factor_name):
    """ 保存所有预测正确的topic
    """
    index = 0
    ...
    for test_topic_id, test_ins, true_level in test_set:
        ...
        f.write(test_topic_id + ' ' + str(y_pred[index][0]) + '\n')
        index += 1
...
def main(group_id):
    ...
    prediction_date = timedelta(hours=4*5)
    target_date = timedelta(hours=8*5)
    ...
    dataset, comment_count_dataset, Bao_dataset, category_count_list = prepare_dataset(group_id, \
        topic_list, gaptime, pop_level, prediction_date, target_date, alpha)
    # 保存那些经过筛选的topic id
    ...
    k = 5
    y_true, y_pred, comment_true, comment_pred = classify(train_set, test_set, k, num_level)
    # evaluate results
    classification_evaluation(y_true, y_pred)
    level_MSE_evaluation(y_true, y_pred)
    #save_correct_predictions(test_set, y_true, y_pred, factor_name = 'author')
    ...
    ratio_accuracy_distribution_plot(y_true, y_pred, test_set, group_id, factor_name='num_authors')
